N1
Tittle: Invitation Creation

Description: Creating an invitation by the user

Precondition:
• The user is authorized
• The user is on the templates page

Postcondition:
The user creates an invitation

Steps:

Open the website: https://invu.ge/
1. Click on the first template labeled: "simple wedding"
2. Fill in at least the required fields on the specific page
3. Click the button: "მოსაწვევის შექმნა"

Expected Result: The user successfully creates an invitation.

N2
Tittle: Invitation Creation Without Required Fields

Description: Creating an invitation by the user without filling in the required fields

Precondition:
• The user is authorized
• The user is on the templates page

Postcondition:
The user cannot create an invitation

Steps:

Open the website: https://invu.ge/
Click on the first template labeled: "simple wedding"
Do not fill in the required fields on the specific page
Click the button: "მოსაწვევის შექმნა"

Expected Result: The system does not allow the user to click the invitation creation button until the required fields are filled in.

N3
Tittle: Invitation Creation Without Authorization

Description: Creating an invitation by the user without authorization

Postcondition:
The user cannot create an invitation

Steps:

Open the website: https://invu.ge/
Click the button: "შაბლონები"
Click on the first template labeled: "simple wedding"

Expected Result: The website prompts the user to authorize in order to create an invitation.

N4
Tittle: Selecting the Multiple Event Time Checkbox

Description: Selecting the multiple event time Checkbox by the user

Precondition:
• The user is authorized
• The user is on the templates page

Postcondition:
The user selects the multiple event time Checkbox

Steps:

Open the website: https://invu.ge/
Click on the first template labeled: "simple wedding"
Click the multiple event time Checkbox

Expected Result: The user successfully selects the multiple event time Checkbox.

N5
Tittle: Selecting the Guests RSVP Permission Checkbox

Description: Selecting the guests RSVP permission Checkbox by the user

Precondition:
• The user is authorized
• The user is on the templates page

Postcondition:
The user selects the guests RSVP permission Checkbox

Steps:

Open the website: https://invu.ge/
Click on the first template labeled: "simple wedding"
Click the guests RSVP permission Checkbox

Expected Result: The user successfully selects the guests RSVP permission Checkbox.

